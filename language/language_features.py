...
def perform_similarity(clip_viz_dense: torch.Tensor, text_embs: torch.Tensor) -> torch.Tensor:
    """
...
def get_hr_feats(
    ...
    high_res_lang = model_super(feat_lang["clip_vis_dense"], feat_lang["res3"], feat_lang["res2"])
...
def load_models(
    high_res_checkpoint: str, lang_model_path: str, device: str = "cuda"
    ...
    model_super = LangSupervisedNet()
    model_super = LangSupervisedNet.load_from_checkpoint(high_res_checkpoint)
...
def parse_args():
    ...
    # Model paths
    parser.add_argument("--high-res-model", type=str, required=True, help="Path to high-resolution model checkpoint")
    parser.add_argument("--lang-model", type=str, required=True, help="Path to language model checkpoint")

    # Input options
    parser.add_argument("--input", type=str, required=True, help="Path to input image or directory of images")
    parser.add_argument("--output-dir", type=str, default=None, help="Directory to save features and visualizations")
...
def main():
    ...
    # Load models
    model_super, model_lang, aug = load_models(args.high_res_model, args.lang_model, device)

    # Process input (single image or directory)
    if os.path.isdir(args.input):
        image_files = sorted(glob.glob(os.path.join(args.input, "*.jpg"))) + sorted(
