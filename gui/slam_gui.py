...
class SLAM_GUI:
    def __init__(self, params_gui=None):
        ...
        if params_gui is not None:
            ...
            # self.background_language = params_gui.background_language
            ...
            self.is_language = params_gui.gaussians.is_language
    ...
    def init_widget(self):
        ...
        self.window = gui.Application.instance.create_window("LangMapping", self.window_w, self.window_h)
        ...
        # language
        self.panel.add_child(gui.Label("Language Rendering Options"))
        chbox_tile_language_geometry = gui.Horiz(0.5 * em, gui.Margins(margin))

        self.language_map_chbox = gui.Checkbox("Language Map")
        self.language_map_chbox.checked = False
        chbox_tile_language_geometry.add_child(self.language_map_chbox)

        self.eplipsoid_language_chbox = gui.Checkbox("Elipsoid Language Shader")
        self.eplipsoid_language_chbox.checked = False
        chbox_tile_language_geometry.add_child(self.eplipsoid_language_chbox)
        self.panel.add_child(chbox_tile_language_geometry)
        ...
        self.in_language_widget = gui.ImageWidget()
        tab_info.add_child(gui.Label("Input Color/Depth/Language"))
        ...
        tab_info.add_child(self.in_language_widget)
    ...
    def receive_data(self, q):
        ...
        if gaussian_packet.gtlangauge is not None:
            lang_map = torch.clamp(gaussian_packet.gtlangauge, min=0, max=1.0) * 255
            lang_map = lang_map.byte().permute(1, 2, 0).contiguous().cpu().numpy()
            if self.is_language:
                lang_map = o3d.geometry.Image(lang_map)
                self.in_language_widget.update_image(lang_map)
            else:
                self.in_language_widget.update_image(o3d.geometry.Image())

        if gaussian_packet.gtdepth is not None:
            depth = gaussian_packet.gtdepth
            depth = imgviz.depth2rgb(depth, min_value=0.1, max_value=50.0, colormap="jet")
    ...
    def render_o3d_image(self, results, current_cam):
        if self.depth_chbox.checked:
        ...
        elif self.language_map_chbox.checked and self.is_language:
            language_feature = (
                (torch.clamp(results["language"], min=0, max=1.0) * 255)
                .byte()
                .permute(1, 2, 0)
                .contiguous()
                .cpu()
            ...
            render_img = o3d.geometry.Image(language_feature)
        elif self.eplipsoid_language_chbox.checked and self.is_language:
            if self.gaussian_cur is None:
                return
            glfw.poll_events()
            gl.glClearColor(0, 0, 0, 1.0)
            ...
            bufferdata = gl.glReadPixels(0, 0, width, height, gl.GL_RGB, gl.GL_UNSIGNED_BYTE)
            img = np.frombuffer(bufferdata, np.uint8, -1).reshape(height, width, 3)
            # print("before flip img shape: ", img.shape)
            # cv2.flip(img, 0, img)
            img = cv2.flip(img, 0)
            # print("after flip img shape: ", img.shape)
            render_img = o3d.geometry.Image(img)
            glfw.swap_buffers(self.window_gl)
        else:
            rgb = (
