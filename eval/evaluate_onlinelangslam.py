    ...
    for k in range(len(positives)):
        ...
        for box in img_ann[positives[k]]["bboxes"].reshape(-1, 4):
            ...
            for cord_list in coord_final:
                if cord_list[0] >= x_min and cord_list[0] <= x_max and cord_list[1] >= y_min and cord_list[1] <= y_max:
                    acc_num += 1
                    flag = 1
                    break
            if flag != 0:
        ...
        show_result(valid_composited.cpu().numpy(), coord_final, img_ann[positives[k]]["bboxes"], save_path)
    return acc_num
...
def process_single_eval(
    ...
    image_name = (
        Path(output_path) / f"{img_name}_{eval_index:0>5}"
    )  # image_name = Path(output_path) / f'{eval_index+1:0>5}'
    ...
    with torch.no_grad():
        ...
        reshaped_sem_feat_15 = F.interpolate(
            sem_feat.permute(0, 3, 1, 2), size=(new_h, new_w), mode="bilinear", align_corners=False
        ).permute(0, 2, 3, 1)
        reshaped_sem_feat_32 = model_online.decode(reshaped_sem_feat_15.flatten(0, 2))
        restored_feat_reshape = model.decode(reshaped_sem_feat_32)
        restored_feat_reshape = restored_feat_reshape.view(lvl, new_h, new_w, -1)
        restored_feat = F.interpolate(
            restored_feat_reshape.permute(0, 3, 1, 2), size=(h, w), mode="bilinear", align_corners=False
        ).permute(0, 2, 3, 1)
    ...
    chosen_iou_list, chosen_lvl_list = [], []
    c_iou_list, c_lvl = activate_stream(
        restored_feat, rgb_img, clip_model, image_name, img_ann, thresh=mask_thresh, colormap_options=colormap_options
    )
...
def evaluate_per_image(
    ...
    model.to(device)
    model.eval()
    model_online = EncoderDecoderOnline(method="mlp", input_dim=32, compressed_dim=15)
    model_online.load_state_dict(torch.load(online_ckpt_path))
    model_online.to("cuda")
    model_online.eval()
    ...
    for eval_index in tqdm(eval_index_list):
        if len(gt_ann[str(eval_index)]) == 0:
            continue
        mean_iou_chosen, acc = process_single_eval(
            feat_dir,
            eval_index,
            image_paths[counter],
            image_shape,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default="room0", help="Replica sequence name")
    parser.add_argument("--clip_model_type", type=str, default="convnext_large_d_320", help="clip model type")
    parser.add_argument("--clip_dim", type=int, default=768)
    parser.add_argument(
        "--root_dir",
        type=str,
        default="/media/saimouli/Data6T/Replica/omni_data_result/room_0_small/2025-03-24-06-23-28/psnr/before_opt",
    ...
    parser.add_argument(
        "--ae_ckpt_dir",
        type=str,
        default="/home/saimouli/Downloads/training_weights_general/epoch=149-step=16500.ckpt",
        help="path to the generalized autoencoder checkpoint",
    )
    parser.add_argument(
        "--online_ae_ckpt",
        type=str,
        default="/media/saimouli/Data6T/Replica/omni_data_result/room_0_small/2025-03-24-06-23-28/online_15_room0.pth",
        help="path to the generalized online autoencoder checkpoint",
    )
