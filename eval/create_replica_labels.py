...
def process_images_labels(args, selected_imgs, selected_idx, output_folder, img_list_path, top_labels, output_name):
    ...
    gt_segmentation_dir = args.seg_file_config.replace("render_config.yaml", "semantic_class")
    ...
    for i, img_file_name in enumerate(selected_imgs):
        ...
        seg_label_path = os.path.join(gt_segmentation_dir, f"semantic_class_{img_numeric_part}.png")
        ...
        success = save_json_labels(
            args.seg_file_config, seg_label_resized, output_json, img_name, selected_idx[i], user_label_names=top_labels
        )
        if success:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--langslam_dir", type=str, help="Root dataset folder contaning online language splatting results"
    )
    parser.add_argument("--langsplat_dir", type=str, help="Root dataset folder contaning LangSplat results")
    ...
    # Randonmly selected images for eval
    ...
    # Folders containing RGB image groundtruth to select image names
    langslam_img_list = get_image_list(os.path.join(args.langslam_dir, "gt"))
    langsplat_img_list = get_image_list(os.path.join(args.langsplat_dir, "gt"))
    ...
    selected_numeric_keys = [key for key in langslam_seed_keys if key in langslam_numeric_map]

    selected_numeric_keys_langpslat = [key for key in langsplat_seed_keys if key in langsplat_numeric_map]
    ...
    # The script assumes you have our langslam and langsplat's results already. Comment the code out if you don't have one of them.
    process_images_labels(
        ...
        img_list_path=os.path.join(args.langsplat_dir, "gt"),
    ...
    process_images_labels(
        ...
        img_list_path=os.path.join(args.langslam_dir, "gt"),
        top_labels=user_label_names,
        output_name=args.output_name,
    )
