...
from language.autoencoder.model import AutoencoderLight, EncoderDecoderOnline, AutoencoderMLP
...
class BackEnd(mp.Process):
    ...
    def _init_language_models(self):
        ...
        if self.config["language"]["language_train"]:
            ...
            if self.is_single_stage:
                ...
                # Compress from 32 -> 15; Decode from 15 -> 32; online trained
            ...
            # For disentanglement mode 3-channel single-stage AE
            self.auto_model = AutoencoderMLP(encoder_hidden_dims, decoder_hidden_dims, clip_dim=768)
            ...
            self.auto_model.load_state_dict(checkpoint)
            ...
            if self.config["language"]["hr_model"]:
                ...
                self.hr_model = LangSupervisedNet.load_from_checkpoint(hr_ckpt_path)
    ...
    def set_hyperparams(self):
        ...
        self.single_thread = (
            self.config["Dataset"]["single_thread"] if "single_thread" in self.config["Dataset"] else False
        )
    ...
    def initialize_map(self, cur_frame_idx: int, viewpoint: Any) -> Dict[str, Any]:
        ...
        for mapping_iteration in tqdm(range(self.init_itr_num), desc="Mapping Iteration"):
            ...
            (
                ...
                render_pkg["radii"],
                render_pkg["radii_lang"],
                render_pkg["depth"],
                render_pkg["opacity"],
                render_pkg["n_touched"],
                render_pkg["n_touched_lang"],
                render_pkg["scale"],
                render_pkg["scale_lang"],
            )
            ...
            if self.config["language"]["language_train"] and not self.is_single_stage:
                ...
                with torch.no_grad():
                    inputs = [{"image": (viewpoint.original_image * 255.0).cuda(), "height": height, "width": width}]
                    ...
                    if viewpoint.gt_lang_feat is None:
                        ...
                        if self.config["language"]["hr_model"]:
                            high_res_lang = self.hr_model(
                                clip_viz_dense["clip_vis_dense"], clip_viz_dense["res3"], clip_viz_dense["res2"]
                            )
                ...
                # [DISABLED in disent mode] Train online autoencoder periodically
                ...
                #         low_dim_coco, online_auto_optimizer, online_auto_scheduler, viz=(mapping_iteration % 300 == 0)
                #     )
            ...
            with torch.no_grad():
                ...
                self.gaussians.max_radii2D_lang[visibility_filter] = torch.max(
                    self.gaussians.max_radii2D_lang[visibility_filter],
                    radii_lang[visibility_filter],
                )
        ...
        self.occ_aware_visibility[cur_frame_idx] = torch.logical_and(n_touched > 0, n_touched_lang > 0).long()
    ...
    def map(
        ...
        for _ in range(iters):
            ...
            radii_acm_lang, n_touched_acm_lang = [], []
            keyframes_opt = []
            ...
            for cam_idx in range(len(current_window)):
                ...
                if self.config["language"]["language_train"] and lang_run:
                    ...
                    gt_lang_feat_resize = F.interpolate(
                        viewpoint.gt_lang_feat.unsqueeze(0), size=(height, width), mode="bilinear", align_corners=False
                    ).squeeze(0)
                ...
                if self.config["language"]["language_train"] and lang_run:
                    scale_loss = l1_loss(scale.detach(), scale_lang)
                    loss_mapping += self.lamda_lang * Ll1_feat + self.lamda_scale * scale_loss

                # Accumulate tensors
                viewspace_point_tensor_acm.append(viewspace_point_tensor)
                visibility_filter_acm.append(visibility_filter)
                radii_acm.append(radii)
                radii_acm_lang.append(radii_lang)
                n_touched_acm.append(n_touched)
                n_touched_acm_lang.append(n_touched_lang)
            ...
            loss_mapping += self.lamda_scale * isotropic_loss.mean()
            ...
            with torch.no_grad():
                ...
                for idx in range(len(current_window)):
                    ...
                    n_touched_lang = n_touched_acm_lang[idx]
                ...
                for idx in range(len(viewspace_point_tensor_acm)):
                    ...
                    self.gaussians.max_radii2D_lang[visibility_filter_acm[idx]] = torch.max(
                        self.gaussians.max_radii2D_lang[visibility_filter_acm[idx]],
                        radii_acm_lang[idx][visibility_filter_acm[idx]],
                    )
    ...
    def color_refinement(self):
        ...
        for iteration in tqdm(range(1, iteration_total + 1), desc="Color Refinement"):
            ...
            # Extract render package components
            image, visibility_filter, radii, radii_lang = (
                render_pkg["render"],
                render_pkg["visibility_filter"],
                render_pkg["radii"],
                render_pkg["radii_lang"],
            ...
            if self.backend_queue.empty():
                ...
                if data[0] == "stop":
                    ...
                    if not self.is_single_stage:
                        ...
                        torch.save(self.online_auto.state_dict(), self.config["language"]["online_ckpt_path"])
